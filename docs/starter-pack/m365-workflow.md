...
## Align the route with capture and delivery

Choose every captured edit or IMPORTANT-only delivery at the monitoring source. A downstream route cannot review changes that were filtered out upstream. Keep sensitive brand/HCP wording and image edits available when the brief requires them. Retain uncertainty in the review queue.

Store saved platform IMPORTANT separately from a new proposed classification. Map `observed_at`, `ai_summary`, `classification_provenance`, reviewer, backup and evidence reference from the documented event input. The included saved-event fixtures are an offline source example. Their local event IDs are not Outlook message IDs; only a real trigger supplies that email identifier.
